...
//es una clase de vector normal como la otra
template<class T> class vector_t {
 public: //metodos
  ...
  vector_t<T>& operator=(const vector_t<T>&); //actua como sobrecarga de =
  ...
  // getters-setters
  T& at(const int);
  T& operator[](const int); //esto era para escribir directamente []
  ...
  // E/S metodos de escritura y lectura
 ...
 private: //atributos
  ...
  void build(void); //funciones para construit la clase 
  void destroy(void); //y destruirla
};
...
//para cambiar una posicion del vector
template<class T> T& vector_t<T>::at(const int i) {
  assert(i >= 0 && i < get_size());
  return v_[i];
}

//sobrecarga del [ ] pa que sea mas comodo
template<class T> T& vector_t<T>::operator[](const int i) {
  return at(i);
}

//hacemos al at que sea constante
